...
using CaProcessing:
    ...
    frame_avg_intensity,
    get_norm,
    frames_min_max_accum_alloc,
    frames_min_max_accum_init!,
    frame_min_max_accum!,
    determine_container_depth,
    make_scale_f,
    make_pixel_lut,
    apply_lut!,
    maxval_min_max_frames,
    median_filter_frames!
using SignalIndices: ndx_to_t, t_to_ndx, clip_ndx, find_all_edge_triggers
using ColorTypes: Gray, RGB, RGB24
using DataStructures: OrderedDict, CircularBuffer, isfull

using FeatherscopeExtraction:
    convert_feather_video_frames,
    convert_featherscope_img!,
    convert_featherscope_rgb,
    open_audio_sync,
    FEATHER_VIDEO_REG,
    FEATHER_SYNC_REG,
    FEATHER_SYNC_HIGH,
    FEATHER_SHUTTER_HIGH,
    sync_searchreg,
    frame_iter_preamble,
    file_triplets,
    open_utinstants
...
using VideoIO: open_video_out, VideoWriter, openvideo, close_video_out!, get_number_frames
import VideoIO
...
function _add_to_min_max_frames!(roi_max, roi_min, img_raw, roi_xr, roi_yr, yb, ye)
    @inbounds for yi = yb:ye
        @simd ivdep for xi in eachindex(roi_xr)
            intensity = reinterpret(
                N6f10,
                convert_featherscope_rgb(img_raw[roi_xr[xi], roi_yr[yi]]),
            )
...
function add_to_min_max_frames!(roi_max, roi_min, img_raw, roi_xr, roi_yr, nt)
    ...
    if nt > 1
        ...
        @inbounds for tno = 1:nt
            ...
            tasks[tno] = @spawn _add_to_min_max_frames!(
                roi_max,
                roi_min,
                img_raw,
                roi_xr,
        ...
        _add_to_min_max_frames!(roi_max, roi_min, img_raw, roi_xr, roi_yr, 1, ny)
    end
end

function exposure_incr!(
    roi_max,
    roi_min,
    min_frames,
    subtracted_maxvals,
...
function determine_if_frame_exposed(img_raw, roi_xr, roi_yr, norm, thr, nt = nthreads())
    frame_intensity = frame_avg_intensity(
        convert_featherscope_rgb,
        UInt64,
        img_raw,
        norm;
...
function feather_video_exposed_medfilter_blocks(
    videof,
    thr,
    filter_npt,
    roi_x = :,
    ...
    frames = reinterpret(
        UInt16,
        convert_feather_video_frames_roi(videof; scratch_dir, roi_x, roi_y),
    )
...
function centerval_8bit_gray_encode(
    vfname,
    buff,
    framerate,
    neg_range = 1.0,
    ...
    for i = 1:nf
        encode_buff .= Gray.(
            reinterpret.(
                N0f8,
                rescale_clamp_brightness.(UInt8, view(buff, :, :, i), clamp_min, clamp_max, e - b) .+
                UInt8(b),
            ),
        )
        write(writer, encode_buff, i - 1)
    end
...
function fv_exp_medfilt_gray_encode(videof, thr, filter_npt, args...; kwargs...)
    outb, exposed_ranges =
        feather_video_exposed_medfilter_blocks(videof, thr, filter_npt, args...; kwargs...)
end

function feather_video_min_frame_planning(
    input_fname,
    thr,
    roi_x = :,
    roi_y = :;
    ...
    fno = 1
    exposure_start = 0
    is_exposed = determine_if_frame_exposed(img_raw, roi_xr, roi_yr, norm, thr, nt)
    nexposed = ifelse(is_exposed, 1, 0)
    in_exposure = nexposed > shutter_delay
    exposure_start = exposure_incr!(
        roi_max,
        roi_min,
        min_frames,
        subtracted_maxvals,
...
function avi_to_tiff_demin(
    ...
    nseg = length(segs)
    nframes = [open_pers[2, i] - open_pers[1, i] + 1 for i = 1:nseg]
...
function append_demind_video_frame!(
    encoder_state,
    exposure_no,
    graybuf,
    img_raw,
    ...
    if encoder_state === nothing
        if fno in exposed_range
            ...
            demin_frame!(graybuf, img_raw, min_frame, encoder_state.lut, roi_xr, roi_yr)
...
function feather_video_encode_demind_segments(
    input_fname,
    roi_x,
    roi_y,
    min_frames,
    ...
    encoder_private_options = (crf = 20, preset = "medium"),
    force_video = false,
)
    ...
    encoder_state, exposure_no = append_demind_video_frame!(
        ...
        graybuf,
        img_raw,
        exposed_ranges[exposure_no],
        min_frames[exposure_no],
        subtracted_maxvals[exposure_no],
        fno,
        roi_xr,
        roi_yr,
        video_names[exposure_no],
        framerate,
        use_gamma_compression,
        container_options,
        container_private_options,
...
function feather_video_read_demin(
    input_fname,
    thr,
    roi_x,
    roi_y,
...
function feather_sync_add_audio(
    syncf,
    out_filenames,
    exposed_ranges,
    sync_frameno,
    ...
    sync_start_time = video_sync_alignment(syncdata, sync_frameno, framerate; kwargs...)
    ...
    for exposure_no = 1:nexposed
        ...
        exposure_sync_start_ndx =
            clip_ndx(t_to_ndx(time_start, fs_sync, sync_start_time), syncl)
        exposure_sync_stop_ndx =
            clip_ndx(t_to_ndx(time_stop, fs_sync, sync_start_time), syncl)
        audio_segment = view(syncdata, 1, exposure_sync_start_ndx:exposure_sync_stop_ndx)
...
function find_matching_syncf(videof)
    ...
    videomatch === nothing &&
        throw(ArgumentError("Could not parse $videof to find its datetime"))
...
function feather_video_read_demin_audio(
    videof::AbstractString,
    syncf::AbstractString,
    thr::Real,
    roi_x,
    roi_y,
    framerate,
    ...
    out_filenames = feather_video_encode_demind_segments(
        videof,
        roi_x,
        roi_y,
        min_frames,
    ...
    sync_exposed_frameno =
        first_exposure_nosync ? exposed_ranges[2][1] : exposed_ranges[1][1]
    try
        feather_sync_add_audio(
            syncf,
            out_filenames,
            exposed_ranges,
            sync_exposed_frameno,
...
function exposure_map_terminate!(
    end_exposure_f::F,
    priv_data,
    exposure_outs,
    exposed_ranges,
...
function exposure_map_loop_body!(
    new_exposure_f::F,
    grow_exposure_f::G,
    end_exposure_f::H,
    nexposed,
    ...
    if img_exposed
        ...
        if in_steady_state
            ...
            exposure_outs = exposure_map_terminate!(
                end_exposure_f,
                priv_data,
                exposure_outs,
                exposed_ranges,
...
function frames_min_max_accum_new_exp!(
    ::Type{S},
    img::AbstractArray{T};
    nt = nthreads(),
) where {S,T}
...
function frames_min_max_accum_grow_exp!(minf, maxf, accf, tasks, rowrange, colranges, img)
    frame_min_max_accum!(minf, maxf, accf, tasks, img, rowrange, colranges)
    minf, maxf, accf, tasks, rowrange, colranges
end

function frames_min_max_accum_end_exp!(
    ::Type{T},
    exposed_range,
    minf,
    maxf,
...
function feather_video_read_demean_write(
    videof,
    thr,
    framerate,
    outdir = pwd();
    ...
    imgs =
        reinterpret(UInt16, convert_feather_video_frames(videof, scratch_dir = scratch_dir))
    img_stack = [view(imgs,:,:,i) for i = 1:size(imgs, 3)]
    new_exposure_f = (img, args...) -> frames_min_max_accum_new_exp!(UInt32, img, args...)
    end_exposure_f =
        (args...) -> frames_min_max_accum_end_exp!(Float32, args...; use_gamma = false)
    thr_scaled = thr * reinterpret(one(N6f10))
    outs, exposed_ranges = exposure_map!(
        new_exposure_f,
    ...
    for (i, ((outT, f, meanf), exposed_range)) in enumerate(zip(outs, exposed_ranges))
        ...
        new_vid_path = joinpath(outdir, demeaned_video_name(videof, exposed_range))
        write_demeaned_video(
            f,
            rawtype(outT),
            new_vid_path,
            img_block,
...
function feather_video_read_demean_grid_write(
    videof,
    thr,
    framerate,
    outdir = pwd();
...
function feather_video_read_demean_write_audio(
    videof::AbstractString,
    syncf::AbstractString,
    thr,
    framerate,
    ...
    vid_paths, exposed_ranges = feather_video_read_demean_write(
        videof,
        thr,
        framerate,
        outdir;
...
function feather_video_read_demean_write_audio(
    ...
    syncpath = find_matching_syncf(videof)
    feather_video_read_demean_write_audio(videof, syncpath, thr, args...; kwargs...)
end

function write_demeaned_grid_video(
    f,
    ::Type{T},
    out_filename,
    img_stack,
    ...
    writer =
        open_video_out(out_filename, framebuff; framerate, scanline_major = true, kwargs...)
    try
        for i in eachindex(img_stack)
            this_img = img_stack[i]
            for j in eachindex(this_img)
...
function streaming_intensities(fname, roi_x = :, roi_y = :; nt = nthreads())
    ...
    fno = 1
    @inbounds intensities[fno] = frame_avg_intensity(
        convert_featherscope_rgb,
        UInt64,
        img_raw,
        norm;
...
"""
function find_first_exposure_edge(
    fname,
    thr,
    roi_x = :,
    roi_y = :;
    ...
    ignore_exposure, is_exposed =
        update_exposure_first_frame(ignore_exposure, intensity, thr)
    ...
    return eof(r) ? nothing : (fno, crossingno)
end

function find_exposed_frame_ranges(
    ...
    ignore_exposure, is_exposed =
        update_exposure_first_frame(skip_exposure_at_start, intensity, thr)
    nexposed = ifelse(is_exposed, 1, 0)
    while !eof(r)
        fno += 1
        read!(r, img_raw)
        ...
        if !is_exposed & (nexposed > shutter_delay) # Leaving exposure
            ...
            start_of_exposure =
                raw_start_of_exposure + ifelse(raw_start_of_exposure == 1, 0, shutter_delay)
...
function find_exposed_frame_ranges(
    ...
    intensity = frame_avg_intensity(identity, UInt64, img_raw, norm; roi_xr, roi_yr, nt)
    ignore_exposure, is_exposed =
        update_exposure_first_frame(skip_exposure_at_start, intensity, thr)
    nexposed = ifelse(is_exposed, 1, 0)
    for fno = 2:nf
...
function avi_to_scaled_gray_video(
    out_filename,
    in_filename,
    framerate;
    scratch_dir = "",
    ...
    encoder_options = (color_range = 2,), force_file_check(out_filename, force)
...
function combine_featherscope_chunks(
    outfile,
    in_files,
    in_frame_ranges = nothing;
    roi_x = :,
    roi_y = :,
)
    ...
    if in_frame_ranges !== nothing
        if length(in_frame_ranges) != length(in_files)
            throw(
                ArgumentError(
                    "in_frame_ranges must be `nothing` or same length as `in_files`",
                ),
            )
        end
    end
    ...
    open(outfile, "w") do io
        for (file_no, file) in enumerate(in_files)
            ...
            while !eof(r) &&
                      in_frame_ranges !== nothing &&
                      local_fno <= last(in_frame_ranges[file_no])
                read!(r, img_raw)
                gray_img .= convert_featherscope_rgb.(view(img_raw, roi...))
                write(io, gray_img)
                fno += 1
...
function maybe_mmap(::Type{T}, sz::NTuple{N,<:Integer}, scratch_dir = tempdir()) where {T,N}
...
function convert_feather_video_frames_roi(
    input_fname;
    scratch_dir = tempdir(),
    roi_x = :,
    roi_y = :,
    ...
    nframes === nothing && error("Container $input_fname does not report number of frames")
    roi_xr, roi_yr = check_slices(img_raw, roi_x, roi_y)
    nx = length(roi_xr)
    ny = length(roi_yr)
    ...
    for fno = 2:nframes
        read!(inputvid, img_raw)
        convert_featherscope_img!(view(imgs,:,:,fno), view(img_raw, roi_xr, roi_yr))
    end
    imgs
end
